  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8 bg-slate-50 min-h-screen">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
        ...
        <select value={selectedId} onChange={(e) => handleProtocolChange(e.target.value)} className="w-full p-4 bg-slate-100 rounded-2xl border-none font-bold text-slate-700 text-lg focus:ring-2 focus:ring-red-500 transition-all">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <input type="number" value={weight} onChange={e => setWeight(Number(e.target.value))} className="w-full p-4 bg-slate-100 rounded-2xl font-bold" /></label>
            <div className="grid grid-cols-2 gap-4">
              <label><span className="text-[11px] font-bold text-slate-500">단위</span>
              <select value={unit} onChange={e => setUnit(e.target.value)} className="w-full p-4 bg-slate-100 rounded-2xl font-bold">
              ...
              <input type="number" step="0.1" value={dose} onChange={e => setDose(Number(e.target.value))} className="w-full p-4 bg-slate-100 rounded-2xl font-bold" /></label>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <label><span className="text-[11px] font-bold text-slate-500">수액량 (ml)</span>
              <input type="number" value={fluidVolume} onChange={e => setFluidVolume(Number(e.target.value))} className="w-full p-4 bg-slate-100 rounded-2xl font-bold" /></label>
              <label><span className="text-[11px] font-bold text-slate-500">투여 시간 (h/day)</span>
              <input type="number" step="0.5" value={infusionHours} onChange={e => setInfusionHours(Number(e.target.value))} className="w-full p-4 bg-slate-100 rounded-2xl font-bold" /></label>
            </div>
            <label className="block"><span className="text-[11px] font-bold text-slate-500">총 치료 기간 (day)</span>
            <input type="number" value={totalDays} onChange={e => setTotalDays(Number(e.target.value))} className="w-full p-4 bg-slate-100 rounded-2xl font-bold" /></label>
          </div>
        </div>

        {/* 결과 카드 */}
        <div className="bg-red-700 text-white p-10 rounded-3xl shadow-2xl flex flex-col justify-between relative">
