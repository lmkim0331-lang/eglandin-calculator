...
function CalculatorContent() {
  ...
  // URL 파라미터(프로토콜 리스트에서 전달된 값) 처리
  useEffect(() => {
    const u = searchParams.get('unit');
    const d = searchParams.get('dose');
    const diseaseName = searchParams.get('disease'); // 질환명
    const protocolInfo = searchParams.get('protocol'); // 기준 프로토콜
    const durationInfo = searchParams.get('duration'); // 권장 기간
  ...
  const reset = () => {
    ...
    setSelectedInfo(null); // 안내문도 초기화
  };

  return (
    <div className="flex flex-col gap-6">
      {/* 1. 상단 프로토콜 안내 박스 (선택 시에만 나타남) */}
      {selectedInfo && (
        <div className="w-full bg-slate-50 border-l-4 border-red-600 p-4 rounded-r-xl shadow-sm animate-in fade-in slide-in-from-top-2 duration-500">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-xl">📌</span>
            <h3 className="text-lg font-bold text-slate-800">적용된 프로토콜: {selectedInfo.name}</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-slate-600">
            <p>• <span className="font-semibold text-slate-900">기준 프로토콜:</span> {selectedInfo.protocol}</p>
            <p>• <span className="font-semibold text-slate-900">권장 투여기간:</span> {selectedInfo.duration}</p>
          </div>
        </div>
      )}
